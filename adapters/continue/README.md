...
Status: experimental, adapter 0.2.0. Configuration reviewed on 2026-09-10; no named-harness versions or platforms certified for this revision. Refs [#49](https://github.com/october-dev/october-bus/issues/49).
...
```sh
october-bus harness config continue --scope my-project --agent continue-reviewer --name "Continue Reviewer"
october-bus doctor --harness continue --scope my-project
```

Configuration location: `.continue/mcpServers/october-bus.json`. The [template](mcp.json.example) contains placeholders; use the generator, not the template verbatim. The generator never edits existing configuration or stores credentials in it. Review and merge just the October Bus entry. Retain the host's own tool approvals and workspace trust controls.
