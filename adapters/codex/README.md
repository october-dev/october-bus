...
Use `check_inbox` with `waitMs=1000` between work steps. For near-real-time awareness outside the model loop, run `october-bus watch --scope <scope> --agent <id>` in a side pane — peer activity prints as NDJSON as it lands — and `october-bus inbox inject --scope <scope> --agent <id>` drains pending messages as prompt-ready text from a hook or wrapper. `october-bus message send --scope <scope> --agent <id> --to <peer> --stdin` sends a reply without MCP. The bridge registers and heartbeats outside the model loop, and attempts retirement when the host closes MCP. A hard kill relies on lease expiry. Each simultaneously connected window/project needs a distinct `--agent` ID; identical IDs deliberately replace the previous execution. Remove this server entry to disconnect, and verify retirement in the Bus before reassigning work.
