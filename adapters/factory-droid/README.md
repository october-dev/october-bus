...
Status: experimental, adapter 0.2.0. Configuration reviewed on 2026-09-10; no named-harness versions or platforms certified for this revision. Refs [#60](https://github.com/october-dev/october-bus/issues/60).
...
```sh
october-bus harness config factory-droid --scope my-project --agent factory-droid-reviewer --name "Factory Droid Reviewer"
october-bus doctor --harness factory-droid --scope my-project
```

Configuration location: `.factory/mcp.json`. The [template](mcp.json.example) contains placeholders; use the generator, not the template verbatim. The generator never edits existing configuration or stores credentials in it. Review and merge just the October Bus entry. Retain the host's own tool approvals and workspace trust controls.
