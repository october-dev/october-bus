...
Status: experimental, adapter 0.2.0. Configuration reviewed on 2026-09-10; no named-harness versions or platforms certified for this revision. Refs [#57](https://github.com/october-dev/october-bus/issues/57).
...
```sh
october-bus harness config kiro --scope my-project --agent kiro-reviewer --name "Kiro Reviewer"
october-bus doctor --harness kiro --scope my-project
```

Configuration location: `.kiro/settings/mcp.json`. The [template](mcp.json.example) contains placeholders; use the generator, not the template verbatim. The generator never edits existing configuration or stores credentials in it. Review and merge just the October Bus entry. Retain the host's own tool approvals and workspace trust controls.
...
Before promotion, run the full [compatibility runbook](../../compatibility/RUNBOOK.md), both directions with an independent harness, denied-tool cases, reconnection/replacement, and exact-candidate evidence with model, launch mode and public sanitized artifacts. [Upstream configuration documentation](https://kiro.dev/docs/mcp/configuration/).
